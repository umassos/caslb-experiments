...
def experiment(times):
    ...
    for _ in range(epochs):
        ...
        for _ in range(times-1):
            carbonPerGB = (minutesPerGB / 60) * np.random.uniform(100, 700)
            scale_rands.append(setGB * (carbonPerGB / eastToWest))
        ...
        scale_list = []
        repeat = T // times
        for i, distances in enumerate(simplex_distance_rands):
            simplex_distance_list.extend([distances] * repeat)
            phi_list.extend([phi_rands[i]] * repeat)
            scale_list.extend([scale_rands[i]] * repeat)
        ...
        scale_list.extend([scale_list[-1]] * (T - len(scale_list)))
        ...
        try:
            ...
            sol, solCost = f.optimalSolution(simplexSequence, simplex_distance_list, tau*scale, scale_list, c_simplex, dim, start_simplex, time_varying=True)
            ...
            adv, adv_gamma_ots, advCost = f.optimalSolution(errordSequence, simplex_distance_list, tau*scale, scale_list, c_simplex, dim, start_simplex, time_varying=True, alt_cost_functions=simplexSequence)
            adv_ots = [gamma.value for gamma in adv_gamma_ots]
            x_adv = adv.reshape((T, dim))

            #################################### get the online PCM solution

            pcm, pcmCost = f.PCM(simplexSequence, weights, scale_list, c_simplex, job_length, phi_list, dim, Lc, Uc, D, tau*scale, start_simplex, time_varying=True)
            ...
            agn, agnCost = f.agnostic(simplexSequence, weights, scale_list, c_simplex, job_length, dim, tau, simplex_distance_list, start_simplex, time_varying=True)

            const, constCost = f.threshold(simplexSequence, weights, scale_list, c_simplex, job_length, phi, dim, Lc, Uc, D, tau*scale, start_simplex, simplex_distance_list, time_varying=True)

            greed, greedCost = f.greedy(simplexSequence, weights, scale_list, c_simplex, job_length, dim, tau, simplex_distance_list, start_simplex, time_varying=True)

            delayGreed, delayGreedCost = f.delayedGreedy(simplexSequence, errordSequence, weights, scale_list, c_simplex, job_length, dim, tau, simplex_distance_list, start_simplex, time_varying=True)

            #################################### get the online CLIP solution
            epsilon = 0.1
            clip0, clip0Cost = c.Clipper(simplexSequence, weights, scale_list, c_simplex, job_length, phi_list, dim, Lc, Uc, D, tau*scale, adv, adv_ots, simplex_distance_list, epsilon, start_simplex, time_varying=True)

            epsilon = 2
            clip2, clip2Cost = c.Clipper(simplexSequence, weights, scale_list, c_simplex, job_length, phi_list, dim, Lc, Uc, D, tau*scale, adv, adv_ots, simplex_distance_list, epsilon, start_simplex, time_varying=True)
    ...
    with open("volatility/volatility_results{}.pickle".format(times), "wb") as f:
